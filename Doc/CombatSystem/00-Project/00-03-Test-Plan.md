...
涉及 Editor、Content、蓝图、关卡和 PIE 的测试优先通过 UE MCP 构造/检查环境，并遵守 [30-01 UE MCP 开发工作流](../30-Tooling/30-01-UE-MCP-Workflow.md)。UE MCP 用于提高场景准备和诊断效率，断言仍落在 Automation、Functional Test、结构化 Combat Event 或 Dedicated 测试中。
...
每个 [20-01](../20-Content/20-01-Example-Skills.md) 示例提供：
...
- Gate 检查同时读取 [00-04](00-04-Decisions-Gaps.md)；到期 Gap 未关闭时，即使功能测试通过也不得把关联 Task 或 Gate 标为完成。
