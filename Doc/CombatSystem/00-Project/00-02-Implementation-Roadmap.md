...
> 文档性质：本文件冻结 M0-M8 的原始 WBS、依赖和 Gate。M0-M8 当前均已完成验收；实际状态只读取 [00-01 开发进度台账](00-01-Progress-Tracker.md)。下段保留的是 2026-08-24 开始实施前的历史基线，不代表当前代码状态。
...
实际执行状态统一记录在 [00-01 开发进度台账](00-01-Progress-Tracker.md)。本文件只定义计划、依赖和验收标准；不得通过修改本文件中的描述暗示任务已经完成。每个 Task 完成、Gate 状态变化、用户提出修正或验收通过时，都必须同步更新进度台账。
...
- 中文注释：新建或实质修改的项目代码已按 [10-01 §10](../10-Architecture/10-01-Scope-Architecture.md#10-生成代码中文注释规范) 注释类、结构、枚举、函数和关键字段；注释与最终行为一致。
- 文档：实际命名/行为回写对应设计文档；开放偏差进入 [00-04](00-04-Decisions-Gaps.md)。
- 评审：没有绕过统一 Damage/Modifier/Order/Projectile 入口的蓝图或临时代码。

Gate 未通过时，只能继续当前里程碑内不依赖失败项的并行任务；不得开始、创建或修改下一里程碑的代码与资产。

Gap 表中的“最迟节点/Task”自动成为对应 Task 的验收条件。到期 Gap 仍为“开放”时，Task 不得标记为已完成，Gate 也不得通过；关闭证据必须写入 [00-01 开发进度台账](00-01-Progress-Tracker.md)。
...
2. 更新 [00-01 开发进度台账](00-01-Progress-Tracker.md)，执行对应 Gate，并整理验收材料。
3. 停止继续开发，向用户提交本阶段结果。
...
实际冻结结论、owner、迁移和测试映射记录在 [90-01 M0 设计冻结](../90-History/90-01-M0-Design-Freeze.md)；执行状态和用户验收仍只记录在 [00-01](00-01-Progress-Tracker.md)。
...
| DEC-007 | P0 Gap 评审 | 关闭或接受 [00-04](00-04-Decisions-Gaps.md) 中 M1 前项 | DEC-001..006 | 每项有 owner、结论和变更位置 |
...
| FND-007 | Scheduler | 最小堆、稳定顺序、generation、3 policy、budget、teardown | FND-004,FND-006 | [10-02](../10-Architecture/10-02-Scheduler-Transactions.md) P0 用例全绿；关闭 GAP-025 |
...
- UE MCP smoke 通过；测试地图/资产的任何 MCP 写入均已回读、编译和保存，具体流程符合 [30-01](../30-Tooling/30-01-UE-MCP-Workflow.md)。
...
- [10-05](../10-Architecture/10-05-Damage-Heal.md) 与 [10-04](../10-Architecture/10-04-Modifier-Attributes-Motion.md) P0 测试全绿。
...
| PERF-701 | 容量基线 | 目标场景、Server frame/带宽/对象/回调计数 | G6,OBS-701 | 达到 [10-08](../10-Architecture/10-08-Data-Network-Observability.md) 决定的预算；关闭 GAP-018 |
...
- UE MCP 从 M1 完成 smoke 后贯穿每个 Editor/资产任务；不同里程碑的推荐用法见 [30-01](../30-Tooling/30-01-UE-MCP-Workflow.md)。
