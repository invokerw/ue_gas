# 10-10 服务器权威单位移动改造与验收

> 文档状态：生产实现、验收反馈修正、SAM Gate 重验与用户验收均已完成。
> 决策/完成日期：2026-09-02。
...
本设计落实 [ADR-043](../00-Project/00-04-Decisions-Gaps.md)：可玩 Combat Unit 统一采用“PlayerController 指挥、AIController Possess、服务器移动、客户端 SimulatedProxy”的拓扑。它强化 ADR-011 和 ADR-038，不改变 Team 与控制权分离、ASC 属性唯一来源或 `combat_v1_rc1` 的战斗结算契约。
...
- [x] SAM-001..009 按依赖实施，`00-01` 保持为唯一状态源。
