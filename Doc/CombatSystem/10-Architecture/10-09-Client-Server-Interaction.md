...
3. 进入范围后取消移动，由服务器 CharacterMovement 按 `RotationRate.Yaw` 逐帧转向；Order 在 `Facing` 等待误差降至与普攻共用的起手容差内（默认 `15°`），重新校验后调用 `TryActivateCombatAbility`。无目标技能跳过转身，详细规则见 [10-07 §3.1](10-07-Order-Movement.md#31-统一转身速率adr-044)。
...
头顶界面的 C++ 父类负责 View 订阅、状态归并、明确的前摇/引导阶段、服务器时间和生命代次校验，再推送只读展示事件。`WBP_CombatOverhead` 负责资源条、状态文字和血量缓降，`WBP_CombatFloatingText` 负责数字动画。跳字 RPC 携带结果发生时的目标 LifeGeneration，重生后丢弃旧生命数字；专用服务器不创建 Widget。展示接口 v2 要求联机双方使用相同构建，拆分与配置入口见 [10-11 头顶 UI](10-11-Overhead-Blueprint-UI.md)。
