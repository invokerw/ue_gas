...
- 等级与经验已接入服务器权威成长快照；六格物品和三个背包格绑定服务器库存快照，金币与六格储藏处通过 owner-only 经济快照接入，规则和事务见 [10-15](10-15-Economy-Shop-Crafting.md)。
- 预览中的英雄、图标、等级、经验和技能数值均为示例，不写入正式战斗定义或当作平衡配置。
- 物品与背包空槽始终保留；ITEM-001 已接入使用、菜单、拖放换槽和场景丢弃；经济扩展提供金币入口、全局商店、自动合成、出售和储藏转移。真实 WBP 几何与输入仍需 PIE 验收。
