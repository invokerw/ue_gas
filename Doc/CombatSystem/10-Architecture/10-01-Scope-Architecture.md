...
- 天赋、技能树和英雄选择系统。商店、经济、物品实例、主动被动、装备背包及场景交互分别由 ECON-001/ITEM-001 接入，见 [10-14](10-14-Item-System.md) 与 [10-15](10-15-Economy-Shop-Crafting.md)；本条“第一版不做”仅描述已冻结的 v1 历史边界。
...
- M0-M8 的 `combat_v1_rc1` 已完成验收；当前代码契约为 `combat_v4_economy_rc1`，由 ITEM-001 物品基础与 ECON-003 单一库存经济扩展共同组成。物品/经济验证状态见对应 Spec 与台账；最近一次完整发布证据见 [90-17 M8 验收记录](../90-History/90-17-M8-Acceptance.md)，该历史记录不自动证明后续工作区修改已回归。
...
- 完整操作闭环见 [30-01 UE MCP 开发工作流](../30-Tooling/30-01-UE-MCP-Workflow.md)。
...
更完整的数据与版本规则见 [10-08](10-08-Data-Network-Observability.md)。

## 7. 第一版硬约束

M0 冻结的跨系统值域和默认值集中在 [90-01 M0 设计冻结](../90-History/90-01-M0-Design-Freeze.md)：TeamId/关系、LifeState/LifeGeneration、Ability Class/Data/Spec 身份、Numeric/RNG v1、Collision/LOS/cm、Native Tag 和 PrimaryAsset identity。下列硬约束的公共字段必须遵循该契约；变更时按 ADR、schema/version 和迁移规则处理。
...
| 代码注释 | 项目自有的新建或实质修改代码必须按 §10 添加可独立理解的中文注释；缺少类型、自定义函数或关键语义说明时不得通过对应 Gate。 |

如果需要打破任一约束，先在 [00-04](../00-Project/00-04-Decisions-Gaps.md) 增加决策记录、替代路径和迁移影响。
...
- 项目自定义函数：在声明处至少简要说明用途；参数、返回值、失败条件、副作用、权限或时序不直观时必须一并说明。
...
- 注释至少应回答“什么场景会用到、会发生什么、边界是什么”。只把成员名翻译成中文，或只写 `Think`、`ExpireAt`、`ActiveGE`、`Spec` 等内部名词，不算有效说明。
- 术语首次出现时说明业务含义。例如 `Think` 是 Modifier 的周期 `OnThink` 回调，`ExpireAt` 是服务器记录的绝对过期时刻，`ActiveGE` 是该 Modifier 用来持续修改属性和授予标签的 Active GameplayEffect。
- 策略枚举的类型注释应先说明“系统在何时读取这个策略”；每个枚举值再说明触发条件和可观察结果。若策略只控制一部分生命周期，还要明确不受它控制的行为。
- 时间相位、层数、覆盖优先级和数值运算容易误解时，使用最小示例辅助说明。示例必须与实现和测试一致，不能替代规则本身。
- 简单类型或字段仍可只写一句话；目标是信息足够，不是统一拉长注释。

例如，“保留现有 Think 相位，只更新层数与 ExpireAt”依赖读者先理解两个内部字段，不够独立。更清楚的说明应表达为：重复施加同一 Modifier 时，刷新总会更新层数和过期时间；该策略只决定下一次周期 `OnThink` 是否重新计时。若在 `0s` 施加、每 `1s` 触发，并在 `1.5s` 刷新，保留相位会在 `2s` 继续触发，重置间隔会在 `2.5s` 再触发。

### 10.3 写法与维护

- 声明处优先使用 `/** ... */` 文档注释；实现内部使用 `// ...` 解释局部决策。
- 函数声明和定义不机械重复同一段注释；公共契约写在头文件，`.cpp` 只补充实现原因和边界。
- 构造函数、析构函数、简单 getter/setter 和其他惯例代码无需例行注释；存在特殊初始化、资源管理或非显然行为时再说明。
- Unreal Engine 的常规生命周期回调、重写函数和接口实现无需重复引擎语义；只有承载项目特有职责、调用顺序或副作用时才注释。
