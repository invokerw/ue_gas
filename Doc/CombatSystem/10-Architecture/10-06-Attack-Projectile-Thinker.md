# 10-06 普攻、法球、Projectile 与 Thinker
...
M4 已按 `AttackTiming Policy v1` 落地公式、上限和转向策略并关闭 [GAP-010](../00-Project/00-04-Decisions-Gaps.md)；完整公式见 [90-07 §4](../90-History/90-07-M4-Order-Attack-Decision.md#4-attacktiming-policy-v1关闭-gap-010)。
...
随机数必须来自 M0 冻结的 Combat RNG v1，上下文包含 RootEventId、DomainTag、AttackHandle/StableSubjectId 和 Ordinal。固定顺序是 impact 合法性、Evasion、Crit、再按 Modifier 稳定顺序执行 proc；未到达的阶段不创建 roll record。完整 keyed roll 和复现字段见 [90-01 M0 设计冻结](../90-History/90-01-M0-Design-Freeze.md#52-combat-rng-v1)。Miss/evade 不调用 Damage，但仍发送 AttackFail/RecordDestroy。Break/SpellBlock 与普攻法球的交互作为具体 Modifier 规则，不隐含在 Projectile 中。
