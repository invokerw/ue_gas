...
bool FCombatReleaseContract::IsSelfConsistent(FString& OutError) const
{
	...
	bool bValid = true;
	bValid &= Require(ContractVersion == 2, TEXT("发布契约版本必须为 2"));
	bValid &= Require(ReleaseId == TEXT("combat_v2_items_rc1"), TEXT("候选发布标识必须为 combat_v2_items_rc1"));
	...
	bValid &= Require(bItemsEnabled && !bEconomyEnabled && !bItemsAndEconomy, TEXT("本契约启用物品，经济保持延期，旧合并字段保留 false"));
	return bValid;
}
