...
void ACombatTestScenarioActor::BeginPlay()
{
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("CombatCameraSmoke")))
		GetWorld()->SpawnActor<ACombatCameraNetworkScenario>();
...
void ACombatTestScenarioActor::StartM7ClientRpcSmoke()
{
	...
	ACombatPlayerController* LocalController = GetWorld()
		? Cast<ACombatPlayerController>(GetWorld()->GetFirstPlayerController()) : nullptr;
...
void ACombatTestScenarioActor::LogSamClientPositions()
{
	...
	const ACombatPlayerController* LocalController =
		Cast<ACombatPlayerController>(GetWorld()->GetFirstPlayerController());
