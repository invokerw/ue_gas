...
bool FCombatSamCommandBindingLifecycleTest::RunTest(const FString& Parameters)
{
	...
	UWorld& World = *Fixture.GetWorld();
	ACombatGameMode* GameMode = World.SpawnActor<ACombatGameMode>();
	ACombatPlayerController* FirstController = World.SpawnActor<ACombatPlayerController>();
	ACombatPlayerController* SecondController = World.SpawnActor<ACombatPlayerController>();
	...
	TestTrue(TEXT("PlayerController possesses a Command Pawn"), Cast<ACombatCharacter>(FirstController->GetPawn()) != nullptr);
